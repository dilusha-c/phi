...
import { Plus, Trash2, Calendar, User, ClipboardList, AlertCircle } from 'lucide-react';
...
const EditActivityPage = () => {
  ...
  useEffect(() => {
    const loadActivityLog = async () => {
      ...
      try {
        ...
        // Map activities back to Leaflet standard coordinates [lat, lon]
        const mappedActivities = data.activities.map(act => {
          ...
          return {
            ...
            patientName: act.patientName || (act.patient ? `${act.patient.firstName || ''} ${act.patient.lastName || ''}`.trim() : ''),
            patient: act.patient?._id || act.patient || null,
            notes: act.notes || '',
            location: {
              coordinates: coords,
        ...
        pushNotification({
          ...
          message: err.message || 'Error fetching activity log.'
        });
        navigate('/activities');
      } finally {
        setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Map coordinates back to [longitude, latitude] for backend GeoJSON standard
      ...
      await activityService.updateActivity(id, payload);
      pushNotification({
        type: 'success',
        title: 'Activity Log Updated',
        message: `Daily activity log was updated successfully.`
      });
      navigate('/activities');
    } catch (err) {
      pushNotification({
        type: 'error',
        title: 'Save Failed',
        message: err.message || 'Unable to update activity log.'
      });
    } finally {
  ...
  if (loading) {
    return <LoadingSpinner label="Loading daily log..." />;
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-slate-200 bg-white/85 p-6 shadow-soft backdrop-blur dark:border-slate-800 dark:bg-slate-900/90">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-bold text-slate-900 dark:text-white">Edit Activity Log</h2>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Modify daily inspector log records and activities.</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-200 dark:border-slate-800 pt-6 pb-12">
          ...
          >
            {isSubmitting ? 'Saving Changes...' : 'Save Log Changes'}
          </button>
        </div>
      </form>
    </div>
